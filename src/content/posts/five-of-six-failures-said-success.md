...
channels:
  - name: linkedin
    status: planned
    note: "I automated publishing my own articles. The code took an afternoon, and it was tested.\n\nIt then failed six separate times over the following day. Five of those failures reported success.\n\nThe worst one: the job flipped an article to published, then got refused permission on the step that actually ships it. My job looks for articles that are scheduled and due. That one wasn't scheduled anymore — it had already marked itself published. So every later run looked, found nothing due, and exited green.\n\nA checkmark every fifteen minutes, over an article that was never going out. Nothing was broken. Nothing alerted. The only symptom was an article that didn't appear, and me eventually wondering why.\n\nI couldn't just reorder the two steps, because the second one needs the first. So now every run starts by asking a different question: is there anything I already marked done that never actually shipped? If a job can half-finish, it has to look for its own wreckage before it looks for new work.\n\nThe thing worth doing today: ask what your green light is asserting. Mine was asserting \"this script ran to completion.\" I had been reading it as \"the thing I wanted has happened.\"\n\nFull write-up, including the other five failures, in the comments."
    comment: "The whole day, in order — the permission that had never been exercised, the two times my own safety guard blocked my own commits, and the article that shipped with no images: https://www.smartdisruptions.com/content/five-of-six-failures-said-success"
  - name: substack
    status: planned
    note: "I automated publishing my own articles. The code was written and tested in an afternoon. Getting it to actually work took a full day and six failures — five of which reported success.\n\nThe one I keep thinking about: the job marked an article published, then got refused on the step that ships it. Because it was no longer scheduled, every run after that found nothing due and exited green. It would have done that forever.\n\nA job that crashes gets looked at. A job that reports success while sitting on a stuck state gets trusted.\n\nEvery one of my six failures was environmental, ordering, or habit. The code was fine the whole time.\n\nhttps://www.smartdisruptions.com/content/five-of-six-failures-said-success"
  - name: x
    status: planned
