...
channels:
  - name: linkedin
    status: planned
    note: "I built a daily job that records my investment portfolio. No server, no database, no bill — a script about a page long, a scheduled task on GitHub running at 22:15 UTC, and a text file committed like any other change. The file is the database. Its history is the audit log.\n\nThat was the fun part. The part worth teaching came a month later, when I had to move it.\n\nIt lived in a repository that also held my notes, and the access token the app used — a password-like string that lets it read and write — was scoped to all of it. It had never done anything wrong. It just could.\n\nI wanted it in its own repository, with a token that could touch only that. Except it was live and recording every day. Break it and I lose days silently.\n\nHere's the order I used, and I'd use it for anything live:\n\n1. Build the new one. Copy everything, change nothing else.\n2. Prove the new one. Run it by hand, then trigger the real scheduled job.\n3. Only then, cut over.\n4. Only then, delete the old one.\n\nThe chart never missed a day. Not because I was careful, but because the order made carefulness unnecessary.\n\nIf you're moving anything live today: never delete-then-hope.\n\nFull write-up in the first comment."
    comment: "The whole breakdown is here — how the pipeline works, the two things I got right by accident, and the twenty minutes I lost to a brand-new repo whose scheduled jobs hadn't registered yet: https://www.smartdisruptions.com/content/a-pipeline-with-no-server"
  - name: substack
    status: planned
    note: "I used to think \"automated\" meant infrastructure. Something running somewhere, which meant something to pay for and something to maintain, which meant it wasn't for a side project.\n\nMy portfolio tracker now records a point every day on its own. It's a script, a scheduled task where the code already lives, and a text file committed like any other change. No server, no database, no bill.\n\nBut the part I'd actually teach is the move. A month in, I had to relocate the whole thing into its own repository, while it was live and recording daily. Build the new one. Prove the new one. Only then cut over, and only then delete the old one.\n\nThe chart never missed a day — not because I was careful, but because the order made carefulness unnecessary.\n\nAnyone can build the thing. Moving something that's live, without losing a day, is where the skill sits.\n\nhttps://www.smartdisruptions.com/content/a-pipeline-with-no-server"
  - name: x
    status: planned
    note: "1/ My portfolio tracker only knew what was true when I opened it. Miss a week and there's a week-shaped hole in the history. I wanted it to record a point every day whether I showed up or not.\n\nThe obvious answer is a server. I didn't build that.\n\n2/ What I built instead: a script about a page long, a scheduled task on GitHub that runs daily at 22:15 UTC, and a text file the daily number gets committed into.\n\nThe file is the database, and its git history is the audit log. No server, no bill.\n\n3/ Two things I got right by accident.\n\nI checked the script's total against the app to the cent before letting it write anything.\n\nAnd I made it fail loudly. A gap in the chart is honest. A stale number is a lie that looks like data.\n\n4/ Then it failed the way these always do. Worked on my laptop, returned nothing from the scheduled job.\n\nThe price source answers from my house and says \"not found\" when GitHub's servers ask. Fix: try one source, then a second, then a third. Still no keys.\n\n5/ A month later I moved it into its own repo, while it was live and recording daily.\n\nBuild the new one. Prove the new one, with a real scheduled run. Only then cut over. Only then delete the old one.\n\n6/ The chart never missed a day. Not because I was careful in the moment, but because the order made carefulness unnecessary.\n\nNever delete-then-hope. Anyone can build the thing; relocating something live without losing a day is where the skill actually sits."
    comment: "Full write-up, including the twenty minutes I lost because a brand-new repo's scheduled jobs don't register instantly: https://www.smartdisruptions.com/content/a-pipeline-with-no-server"
heroImage: /images/content/a-pipeline-with-no-server-hero.webp
heroImageAlt: "A two-column comparison: a server, a cron host, a database and a monthly bill on one side; a script, a timer, a text file and $0 on the other."
